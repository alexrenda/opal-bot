build/bot.js: build/web/chat.js tsconfig.json $(shell python -c "import json; print(' '.join(json.loads(' '.join(filter(lambda x: '//' not in x, open('tsconfig.json'))))['files']))")
	node node_modules/opal-transformer/build/main.js tsconfig.json

build/web/chat.js: web/chat.ts
	tsc -p web
	cp -R web build
